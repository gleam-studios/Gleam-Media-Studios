...
| `agent/` | Agent 资源总目录；当前剧本工作流在 `agent/script-agent/` |
...
1. 在服务器上克隆仓库，进入仓库根目录，`npm ci`，`npm run build`（构建结束会自动把 `agent/` 拷入 `.next/standalone/`，供 `output: "standalone"` 运行时读取）。
...
4. **Watch Paths**（可选）：Root 已是仓库根时一般无需额外配置；如果平台设置了自定义 watch 规则，确保覆盖 **`app`**、**`components`**、**`lib`**、**`agent`**。
