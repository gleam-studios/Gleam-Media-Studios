...
应用基于 **Next.js**（`web/`），适合本地开发与服务器部署。

## 仓库结构

| 目录 | 说明 |
|------|------|
| `web/` | Next.js 前端与 API Routes（开发与部署入口） |
| `agent/` | 提示词、模板等 Agent 资源 |
| `knowledge/`、`skills/` | 知识库与技能说明（供服务端拼装上下文） |
| `data/projects/` | 项目 JSON（默认不提交，见 `.gitignore`） |
| `services/wattpad-api/` | 可选 Wattpad 相关 FastAPI 服务（与 `web` 的 `npm run dev` 一同拉起） |
...
1. 在服务器上克隆仓库，`cd web`，`npm ci`，`npm run build`（构建结束会自动把 `agent/`、`knowledge/`、`skills/` 拷入 `web/.next/standalone/`，供 `output: "standalone"` 运行时读取）。
...
4. Wattpad 能力：在 **`services/wattpad-api`** 下用 `uvicorn` 自行托管，并将 **`WATTPAD_API_URL`** 指向该服务（见 `web` 内相关 API Route）。

### Zeabur

1. **服务类型**：从 Git 部署 **Node.js** 服务即可（zbpack 会识别 Next.js）。
2. **Root Directory** 设为 **`web`**（本仓库应用入口在该子目录）。
3. **构建 / 启动**：一般无需改命令；使用仓库默认的 `npm run build`、`npm run start` 即可。平台注入的 **`PORT`** 会被 `next start` 使用。
4. **Watch Paths**（可选）：若只把 Root 设在 `web`，建议在服务设置里让下列路径的变更也触发重新部署，否则只改提示词可能不会发新版：例如 **`web`**、**`agent`**、**`knowledge`**、**`skills`**（写法与 Zeabur 文档中的 watch 规则一致）。
