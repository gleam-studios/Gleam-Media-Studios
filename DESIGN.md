...
- **`/chat`** — **standalone Agent chat** (not tied to `projects`): topbar `返回首页` + tagline `对话` + `会话` toggle. Body is full-height `ChatWorkspace` — left rail (LLM 只读摘要 + 全站 Skill 勾选 + 作图提示), centre message stream + composer **or Skill 表单面板**, optional right session list. **Dual mode**: when the selected Skill pack exposes `interface/input.json`, the centre column switches to **`SkillFormPanel`** (vertical stack: 填写需求 card → 生成结果 card; no message composer; results are ephemeral React state, not persisted to `chat_conversations`). Skills without `inputSchema` keep the bubble stream + `ChatComposer`. Data: `chat_conversations` per user; **全站 Skill** in `site_skill_packs` (upload/delete in `/settings` → Skill tab, admin only); **LLM** 共用 `site_settings.llm`；作图读 `image_workspace`。Server `POST /api/chat/agent` runs tool loop (`generate_image` → `lib/image-generate`); form mode uses `POST /api/skills/run`.
